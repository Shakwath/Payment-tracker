import { DollarSign, CheckCircle2 } from "lucide-react";
import { INITIAL_TEACHER_SALARIES } from "../../../data/mockData";

const SalaryHistoryPage = () => {
  return (
    <div className="space-y-6">
      <div className="p-6 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/90 dark:border-slate-800 shadow-sm">
        <div className="flex items-center gap-2 text-emerald-500 font-mono text-xs font-bold uppercase mb-1">
          <DollarSign className="h-4 w-4" />
          <span>Teacher Hub</span>
        </div>
        <h1 className="text-2xl font-black text-base-content tracking-tight">Personal Salary Disbursement History</h1>
        <p className="text-xs text-base-content/60 mt-0.5">
          Record of monthly salary statements, bonuses, and bank direct deposits.
        </p>
      </div>

      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/90 dark:border-slate-800 shadow-sm overflow-hidden">
        <table className="w-full text-left border-collapse text-xs">
          <thead>
            <tr className="bg-slate-50/80 dark:bg-slate-950/80 border-b border-slate-200 dark:border-slate-800 text-[11px] font-mono uppercase font-bold text-base-content/60">
              <th className="py-4 px-6">Month</th>
              <th className="py-4 px-6">Base Salary</th>
              <th className="py-4 px-6">Bonus</th>
              <th className="py-4 px-6">Net Amount</th>
              <th className="py-4 px-6 text-right">Status</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-200/70 dark:divide-slate-800/70">
            {INITIAL_TEACHER_SALARIES.map((s) => (
              <tr key={s.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-950/50">
                <td className="py-4 px-6 font-bold text-base-content">{s.month}</td>
                <td className="py-4 px-6 font-mono">৳ {s.baseSalary.toLocaleString()}</td>
                <td className="py-4 px-6 font-mono text-emerald-500">+৳ {s.bonus.toLocaleString()}</td>
                <td className="py-4 px-6 font-mono font-black text-purple-600 dark:text-purple-400">৳ {s.netPayable.toLocaleString()}</td>
                <td className="py-4 px-6 text-right">
                  <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full font-mono font-bold text-[10px] ${
                    s.status === "Paid" ? "bg-emerald-500/15 text-emerald-500" : "bg-amber-500/15 text-amber-500"
                  }`}>
                    {s.status}
                  </span>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
    </div>
  );
};

export default SalaryHistoryPage;
